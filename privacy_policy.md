...
AirPulse uses Apple HealthKit to access heart rate data and workout-related data. This may include:
...
- the names of connected Bluetooth audio devices made available by iOS;
- whether a supported audio route is active; and
- technical tracking state such as whether tracking is active or paused.
...
AirPulse stores local diagnostic logs to help with troubleshooting and technical analysis. These logs may contain timestamps, launch information, error messages, warnings, workout state, connectivity state, heart rate processing events, and location-filter related runtime messages.
...
Your use of Apple services is subject to Apple's own terms and privacy documentation.
